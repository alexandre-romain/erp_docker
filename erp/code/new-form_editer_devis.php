<?php 
$sql = "SELECT  coment, explic, client_num FROM " . $tblprefluc ."devis WHERE num_dev = $num_dev";
$req = mysql_query($sql) or die('Erreur SQL !<br>'.$sql.'<br>'.mysql_error());
$data = mysql_fetch_array($req);
$num = $data['client_num'];
$coment = $data['coment'];
$explic = $data['explic'];
$sql = "SELECT " . $tblprefluc ."cont_dev.num, quanti, uni, article, tot_art_htva, to_tva_art tva
        FROM " . $tblprefluc ."cont_dev RIGHT JOIN " . $tblprefluc ."article on " . $tblprefluc ."cont_dev.article_num = " . $tblprefluc ."article.num
...
$sql = " SELECT SUM(tot_art_htva) FROM " . $tblprefluc ."cont_dev WHERE dev_num = $num_dev";
$req = mysql_query($sql) or die('Erreur SQL !<br>'.$sql.'<br>'.mysql_error());
?>
...
<tr> 
          ...
          <td class="texte0" colspan="6"> 
					<?php
					$rqSql = "SELECT num, article, prix_htva, uni FROM " . $tblprefluc ."article WHERE actif != 'non' ORDER BY article,prix_htva";
        ...
        </td><td class="texte0" colspan="6"><input name="quanti" type="text" id="quanti" value="1" size="6">
        </td class="texte0"></tr>
		            <tr>
              <td class="texte0">PV (HTVA)</td>
              <td class="texte0"><input name='PV' type='text' id='PV' size='6'>
                &euro; </td>
            </tr>
        <tr> 
      ...
      <form action="new-dev_fin.php" method="post" name="fin_dev">
<br>
<table class="boiteaction">
  <caption>
  ...
  <tr>
  <td class="submit">dénomination (30 caract. max.)<br /><input name="explic" type="text" id="explic" value="<?php echo $explic; ?>" size="40" maxlength="30"/></td>
  </tr>
  <tr>
  <td class="submit"><?php echo $lang_ajo_com_dev ?></td>
  </tr>
  <td class="texte0" ><div align="center"><textarea name="coment" cols="45" rows="3"><?php echo $coment; ?></textarea></div></td>
  </tr>
  <tr>
  <td class="submit"><input type="submit" name="Submit" value='<?php echo $lang_ter_enr; ?>'></td>    
  </tr>
</table>
