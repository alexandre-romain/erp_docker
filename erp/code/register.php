...
if($pass != $pass2) {
	...
	if ($test > 0) { 
		$message = "Erreur : Le login existe deja";
		include('form_utilisateurs.php');
		exit;
	}
	$pass_crypt = Crypter($pass, $cle);
	$login2 = strtolower($login2);
	...
	$message = "$prenom $nom $lang_est_enr $login2.";
	//Si l'utilisateur à des droits restreints, on choisi le clients qu'il peut gérer.
	if ($dev == r || $com == r ||$fact == r) { 
		$message = "$lang_don_rest$login2 $lang_choi_cli_enr";
		include_once("form_choisir_client.php");  
	}
	//Sinon on redirige vers la page de création
	else {
		include("form_utilisateurs.php");
	} 
}
