...
<?php
 ...
 $sql2="SELECT * FROM `" . $tblpref ."article` WHERE cat = $data[id_cat] and actif != 'non' ORDER BY " . $tblpref ."article.marque," . $tblpref ."article.article";
 ...
 $article=  addslashes($data2['article']);
  if ($data2[marge] != 0) {
 echo"articl[\"p$i\"][\"$v\"] = new Array(\"$data2[num]\", \"$data2[marque] $article | PA : $data2[prix_htva] euros | Marge : $data2[marge] % | Stock : $data2[stock]\");\n";
 } else {
 echo"articl[\"p$i\"][\"$v\"] = new Array(\"$data2[num]\", \"$data2[marque] $article | PV : $data2[prix_htva] euros\");\n";
 }
 $v= $v+1;
 }
 $i=$i +1;
 ...
 $article=  addslashes($data3['article']);
 if ($data3[marge] != 0) {
 echo"articl[\"p$j\"][\"$v\"] = new Array(\"$data3[num]\", \"$data3[marque] $article | PA : $data3[prix_htva] euros | Marge : $data3[marge] % | Stock : $data3[stock]\");\n";
 } else {
 echo"articl[\"p$j\"][\"$v\"] = new Array(\"$data3[num]\", \"$data3[marque] $article | PV : $data3[prix_htva] euros\");\n";
 }
 $v= $v+1;
 }
 ?>
