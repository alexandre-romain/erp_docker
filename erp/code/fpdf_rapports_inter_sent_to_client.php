...
$sql_inter ="SELECT t.name as nom_inter, c.nom as cnom, c.rue as crue, c.numero as cnumero, c.ville as cville, c.cp as ccp, c.num_tva as cnum_tva, c.tel as ctel, c.mail as cmail, c.num_client as cnumclient, t.date_due as tdatedue, t.fact_num as tfactnum, t.mail_contact as mail_contact";
...
if ($results_inter->mail_contact == '' || $results_inter->mail_contact == 'NULL') {
	$adress_mail=$results_inter->cmail;
}
else {
	$adress_mail=$results_inter->mail_contact;
}
...
$to = $adress_mail; 
$from = "info@fastitservice.be"; 
$subject = "Rapport D'intervention"; 
$message = "Ci-joint, vous trouverez le rapport d'intervention.";
...
// message
/*$body .= "--".$separator.$eol;
$body .= "Content-Type: text; charset=\"iso-8859-1\"".$eol;
...
header('Location: ./ticket.php?num_ticket='.$inter.''); 
?>