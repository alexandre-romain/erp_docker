...
$num_doc=isset($_GET['num_bon'])?$_GET['num_bon']:"";
...
//pour la date
$sql = "select coment, tot_htva, tot_tva, DATE_FORMAT(date,'%d/%m/%Y') AS date_2,DATE_FORMAT(date,'%Y%m%d') AS date_3 from " . $tblpref ."bon_comm where num_bon = $num_doc";
...
$sql1 = "SELECT num_client, mail, nom, nom2, rue, numero, boite, ville, cp, num_tva, tel AS tel_client, fax AS fax_client FROM " . $tblpref ."client RIGHT JOIN " . $tblpref ."bon_comm on client_num = num_client WHERE  num_bon = $num_doc";
...
//fin js
	...
	$this->SetY(-28);
	$this->SetX(12);
	$this->MultiCell(30,4,"Pour Accord,",0,C,0);	
	...
	$this->SetY(94);
	$this->SetX(12);
	$this->Cell(85,6,"Bon de Commande n° : $date_num_doc-$num_doc",0,0,'C',0);
...
$sql = "SELECT " . $tblpref ."cont_bon.num, " . $tblpref ."cont_bon.article_name, reference, marque, recupel, reprobel, bebat, auvibel, garantie, quanti, uni, article, taux_tva, prix_htva, p_u_jour, tot_art_htva FROM " . $tblpref ."cont_bon RIGHT JOIN " . $tblpref ."article on " . $tblpref ."cont_bon.article_num = " . $tblpref ."article.num WHERE  bon_num = $num_doc";
...
$pdf->SetY(235);
$pdf->SetX(13);
$pdf->Cell(134,3,"L'acceptation du présent document entraine l'acceptation par le client des conditions générales imprimées au verso ou fournies en annexe.",0,0,'C',0);
...
$nom = str_replace("\\",'_',$nom);
$nom = str_replace("'",'_',$nom);
$destination .=retirer_accent($nom);
$destination .=".pdf";
