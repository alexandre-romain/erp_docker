...
$num_doc=isset($_GET['num_bl'])?$_GET['num_bl']:"";
...
//pour la date
$sql = "select coment, tot_htva, tot_tva, DATE_FORMAT(date,'%d/%m/%Y') AS date_2,DATE_FORMAT(date,'%Y%m%d') AS date_3 from " . $tblpref ."bl where num_bl = $num_doc";
...
$sql1 = "SELECT num_client, mail, nom, nom2, rue, numero, boite, ville, cp, num_tva, tel AS tel_client, fax AS fax_client FROM " . $tblpref ."client RIGHT JOIN " . $tblpref ."bl on client_num = num_client WHERE  num_bl = $num_doc";
...
//fin js
	...
	$this->SetY(94);
	$this->SetX(12);
	$this->Cell(85,6,"Bon de Livraison n° : $date_num_doc-$num_doc",0,0,'C',0);
...
$sql = "SELECT ".$tblpref."cont_bl.num, ".$tblpref."cont_bl.article_name, reference, marque, recupel, reprobel, bebat, auvibel, garantie, serial, quanti, uni, article, taux_tva, prix_htva, p_u_jour, tot_art_htva FROM ".$tblpref."cont_bl RIGHT JOIN ".$tblpref."article on ".$tblpref."cont_bl.article_num = ".$tblpref."article.num WHERE  bl_num = $num_doc";
...
$apayer = $tot_tva_inc; // c'est un bl
