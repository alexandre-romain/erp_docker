...
include("../include/config/common2.php");
require("../include/config/var.php");	
require("../include/configav.php");	
require_once("../include/language/$lang.php");
$num_nc=isset($_REQUEST['nc'])?$_REQUEST['nc']:"";
$num_cli=isset($_REQUEST['cli'])?$_REQUEST['cli']:"";	 
$date_day=date('d-m-Y');

//Recupération Infos NC
$sql_total = "SELECT SUM(p_u_jour)*quanti AS total FROM ".$tblpref."cont_nc WHERE nc_num='".$num_nc."' ORDER BY total";
$req_total = mysql_query($sql_total) or die('Erreur SQL !<br>'.$sql_total.'<br>'.mysql_error());
$data_total = mysql_fetch_array($req_total);
$total_htva = $data_total[total];
...
//Debut Infos Clients
$sql1 = "SELECT num_client, mail, nom, nom2, rue, numero, boite, ville, cp, num_tva, tel AS tel_client, fax AS fax_client"; 
$sql1.= " FROM ".$tblpref."client"; 
$sql1.= " WHERE num_client=".$num_cli."";
...
//fin js
	...
	//déco date
	$this->RoundedRect(113, 94, 85, 6, 3, 'DF','1234');	
	...
	function imprimer_contenu($entrep_nom,$social_ville,$social_rue,$social_cp,$tel,$fax,$tva_vend,$compte,$mail,$web,$num_doc,$date_num_doc,$date_doc,$echeance,$total_htva,$total_tva,$coment,$tot_tva_inc,$num_client,$nom,$nom2,$rue,$numero,$boite,$ville,$cp,$num_tva,$mail_client,$tel_client,$fax_client,$date_day,$num_nc)
	{
		global $date_day;
		global $num_nc;
	//la référence client
	...
	$this->SetY(94);
	$this->SetX(12);
	$this->Cell(85,6,"Note de crédit n° : ".$num_nc,0,0,'C',0);

	//Date
	$this->SetFont('Arial','',10);
	$this->SetY(94);
	$this->SetX(113);
	$this->Cell(85,6,"Date : ".$date_day,0,0,'C',0);
...
$total_bebat = '0.00';
		...
		while($data = mysql_fetch_array($req))
    	{
			...
			$pdf->Ln(1);
			$sql2 = "SELECT reference, marque, recupel, reprobel, bebat , garantie, serial, uni, article, taux_tva, prix_htva, p_u_jour, quanti"; 
			$sql2.=" FROM ".$tblpref."cont_nc"; 
			$sql2.=" RIGHT JOIN ".$tblpref."article on ".$tblpref."cont_nc.article_num = ".$tblpref."article.num ";
			$sql2.=" WHERE nc_num = ".$num_nc."";
...
$tot_tva_inc = floatval($total_ttc);
...
$destination = "./output/";
$destination .= $date_nc."-".$num_nc;
$destination .= " Note de credit";
$destination .= " ";
